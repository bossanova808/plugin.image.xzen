...
def AddPhotoThumb(photo, numberOfItems=0, downloadKey=""):
    ...
    try:
        ...
        for key, value in ZEN_DOWNLOAD_QUALITY.items():
        ...
        log("AddPhotoThumb: [" + str(photo.Id) + "] title: [" + str(title) + "] url: [" + str(
            url) + "] urlThumb: [" + str(urlThumb) + "]")
...
def BuildMenuPopSets(type="Gallery", offset=0):
    ...
    if type == "Gallery":
        AddNextPageLink(POPGALLERIES, offset + LIMIT)
    else:
        AddNextPageLink(POPCOLLECTIONS, offset + LIMIT)
...
def BuildMenuRecentSets(type="Gallery", offset=0):
    ...
    if type == "Gallery":
        AddNextPageLink(RECENTGALLERIES, offset + LIMIT)
    else:
        AddNextPageLink(RECENTCOLLECTIONS, offset + LIMIT)
